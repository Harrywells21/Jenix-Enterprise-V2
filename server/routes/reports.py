...
def _health_status(value):
    """Shared 3-tier status label (spec 3.3: warning 85%, critical 95%),
    used by both the single-machine and fleet-wide PDF health tables so
    identical data always yields an identical verdict."""
    ...
    for md in machines_data:
        ...
        if metrics:
            ...
            health_data = [
                ...
                ["CPU Usage",  f"{avg_cpu:.1f}%",  _health_status(avg_cpu)],
                ["RAM Usage",  f"{avg_ram:.1f}%",  _health_status(avg_ram)],
                ["Disk Usage", f"{avg_disk:.1f}%", _health_status(avg_disk)],
            ]
