"""
JENIX Scheduler — runs automated scans on schedule.
"""
import asyncio
...
async def _run_scheduled_scan(schedule_id: int):
    from db import SessionLocal, Schedule, Machine, AuditLog
    ...
    try:
        ...
        if not s or not s.is_active:
            return
        m = db.query(Machine).filter(Machine.id == s.machine_id).first()
        if not m or m.status != "online":
            return
        print(f"[scheduler] Running scheduled {s.scan_type} on {m.hostname}")
        from db import Command
        cmd = Command(machine_id=m.id, type=s.scan_type, status="pending")
        db.add(cmd); db.commit(); db.refresh(cmd)
        sent = await send_command(m.token, {"cmd": s.scan_type, "cmd_id": cmd.id})
        if sent:
            cmd.status = "running"
        else:
            cmd.status = "failed"
        ...
        log = AuditLog(
            machine_id=m.id, action="scheduled_scan",
