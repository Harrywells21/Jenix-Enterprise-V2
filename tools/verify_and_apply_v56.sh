...
echo "=== SECTION 1: Locate the pending narrative/fingerprint patch script ==="
PATCH_SCRIPT=$(find ~/Desktop/jenix -maxdepth 4 -iname "*.py" -newer routes/audit_trail_report.py 2>/dev/null | grep -iv "\.bak" | grep -i "narrative\|fingerprint\|audit_trail_patch\|patch" | head -1)
if [ -z "$PATCH_SCRIPT" ]; then
  echo "[NOT AUTO-FOUND] Could not confidently auto-identify the pending patch script."
  echo "Recent .py files anywhere under ~/Desktop/jenix (last 48h), for manual pick:"
  find ~/Desktop/jenix -iname "*.py" -mmin -2880 2>/dev/null | grep -iv "\.bak"
  echo "[ABORT] Re-run this section after telling me the correct filename, or paste the script's own content if you still have it in scrollback."
  exit 1
fi
echo "Found candidate patch script: $PATCH_SCRIPT"
echo "--- contents (for the record) ---"
cat "$PATCH_SCRIPT"
echo "--- end contents ---"

echo ""
echo "=== SECTION 2: Run the patch script ==="
python3 "$PATCH_SCRIPT"
PATCH_EXIT=$?
echo "Patch script exit code: $PATCH_EXIT"
if [ $PATCH_EXIT -ne 0 ]; then
  echo "[ABORT] Patch script exited non-zero (likely an anchor-assert failure — 0 or 2+ matches). Do NOT proceed. Paste this output back as-is."
  exit 1
fi

echo ""
echo "=== SECTION 3: py_compile check on both files ==="
python3 -m py_compile routes/audit_trail_report.py routes/reports.py
echo "py_compile exit: $?"

echo ""
echo "=== SECTION 4: real import test ==="
source ~/Desktop/jenix/server_venv/bin/activate
python3 -c "import routes.reports; print('IMPORT OK')"
echo "import test exit: $?"

echo ""
echo "=== SECTION 5: write scripted restart (file first, syntax-checked, never inline) ==="
...
echo ""
echo "=== SECTION 7: real POST /api/reports/audit against the SAME real 72-row dataset ==="
...
echo ""
echo "=== SECTION 8: download the regenerated PDF via the confirmed download route ==="
...
echo ""
echo "=== SECTION 9: real GET /api/reports/audit/csv ==="
